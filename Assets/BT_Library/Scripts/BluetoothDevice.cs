...
namespace TechTweaking.Bluetooth
{
	...
	public class BluetoothDevice
	{
		...
		private const string BRUTAL_CONNECT = "brutal_connect";
		...
		internal void initDeviceAsStruct_Name (string name)
		{
		...
		internal void initDeviceAsStruct_MacAddress (string mac)
		{
			this.btConnectionMode = BtConnectionMode.UsingMac;
		...
		public void connect (int attempts, int time, bool allowDiscovery)
		{
			...
			if (!isDeviceReady ())
				return;
			commitData ();
			javaBtConnection.Call (CONNECT, attempts, time, allowDiscovery);
		}

		/* Brutal Connection
		// #ifndef DOXYGEN_SHOULD_SKIP_THIS
		public void brutal_connect ()
		{
			
			this.brutal_connect (10);
		}
		

		public  void brutal_connect (int tries)
		{
			...
			javaBtConnection.Call (BRUTAL_CONNECT, tries,1000,true);
		}
		
		public void brutal_connect(int attempts, int time)
		{
			if (!isDeviceReady())
				return;
			commitData();
			
			javaBtConnection.Call (BRUTAL_CONNECT, attempts,time,true);
		}


		public void brutal_connect(int attempts, int time,bool allowDiscovery)
		{
			...
			javaBtConnection.Call (BRUTAL_CONNECT, attempts,time,allowDiscovery);
		}
*/////Brutal connection end
